...
def read_weight():
    # DT: GPIO5, SCK: GPIO6（実際の接続ピンに合わせて変更）
    hx = HX711(dout_pin=5, pd_sck_pin=6)
    # 初期化とキャリブレーション
    hx.set_reading_format("MSB", "MSB")
    hx.set_reference_unit(230) #キャリブレーション値（後述）
    hx.reset()
    hx.tare()  # 重さの初期化（0点調整)
    # 重量読み取り
    try:
        weight = hx.get_weight(5)
        print(f"読み取った重量: {weight:.2f} g")
        hx.power_down()
        hx.power_up()
        return weight
...
def hx711_add_event_detect(self, hx711_instance, event_callback):
